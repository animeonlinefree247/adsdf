...
# ── Límite de episodios por run (9999 = sin límite efectivo) ──
MAX_EPISODES_PER_RUN = 9999
...
async def scrape_episodes_servers_for_ficha(page, ficha: dict, ficha_checkpoint: dict) -> list:
    ...
    for ep in pending:
        if processed_this_run >= MAX_EPISODES_PER_RUN:
            log(f"  ⏸ Límite de {MAX_EPISODES_PER_RUN} eps/run — guardando checkpoint")
            break
